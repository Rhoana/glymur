"""
A read-only JPEG 2000 wrapper, that reads the data from a memory buffer, instead of disk.
Based on:
...
from .buffer_object import ArrayAsBuffer

DUMMY_FILENAME = 'MEMORY_BUFFER.jp2'

class Jp2kMem(Jp2kBox):
    """JPEG 2000 file.
    ...
    def __init__(self, buffer, shape=None, **kwargs):
        """
        Only the buffer parameter is required in order to read a JPEG 2000
        file.

        Parameters
        ----------
        buffer : ndarray of bytes
            compressed image data buffer to read
        # TODO - need to verify which parameters are needed
        shape : tuple
        ...
        assert(len(buffer.shape) == 1 and buffer.shape[0] > 0 and buffer.dtype == np.uint8)
        self.buffer = np.ascontiguousarray(buffer)
    ...
    def __repr__(self):
        msg = "glymur.Jp2kMem('{0}')".format(hex(id(self.buffer)))
        return msg

    def __str__(self):
        metadata = ['Memory address:  ' + os.path.basename(hex(id(self.buffer)))]
        if len(self.box) > 0:
            for box in self.box:
    ...
    def parse(self):
        ...
        bptr = ArrayAsBuffer(self.buffer)
        read_buffer = bptr.read(2)
        ...
        if (((box_length != 12) or (box_id != b'jP  ') or
            ...
            msg = '{buffer} is not a JPEG 2000 file.'
            msg = msg.format(buffer=hex(id(bptr)))
            raise IOError(msg)

        # Back up and start again, we know we have a superbox (box of
        # boxes) here.
        bptr.seek(0)
        self.box = self.parse_superbox(bptr)
        self._validate()
    ...
    def _read_openjp2(self, rlevel=0, layer=None, area=None, tile=None,
        ...
        with ExitStack() as stack:
            if re.match("2.1", version.openjpeg_version):
                stream = opj2.stream_create_memory_stream(self.buffer)
...
#        return lst
    ...
    def get_codestream(self, header_only=True):
        ...
        bptr = ArrayAsBuffer(self.buffer)
        if self._codec_format == opj2.CODEC_J2K:
            codestream = Codestream(bptr, self.length,
                                    header_only=header_only)
        else:
            box = [x for x in self.box if x.box_id == 'jp2c']
            bptr.seek(box[0].offset)
            read_buffer = bptr.read(8)
            ...
            if box_length == 0:
                ...
                box_length = os.path.getsize(bptr.name) - bptr.tell() + 8
            elif box_length == 1:
                # Seek past the XL field.
                read_buffer = bptr.read(8)
                box_length, = struct.unpack('>Q', read_buffer)
            codestream = Codestream(bptr, box_length - 8,
                                    header_only=header_only)
